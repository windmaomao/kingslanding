...
var RestController = function(path, modelName) {
    ...
    // define populates to new field
    var listProjection = function(refField, field) {
        return function(req, item, cb) {
            var obj = _.assign({}, item._doc);
            obj[field] = item[refField];
            obj[refField] = item[refField]._id;
            cb(null, obj);
        };
    }

    for(var key in SchemaDef.populates) {
        restifyOptions.populate = key;
        restifyOptions.detailProjection =
            restifyOptions.listProjection =
            listProjection(key, SchemaDef.populates[key]);
    }

    // define populate option
    if (SchemaDef.options.populate) {
        restifyOptions.populate = SchemaDef.options.populate;
    }

    // restify route
    var Rest = restifyMongoose(Model, restifyOptions);
