...
function configRoutes(server, config) {
    ...
    _.each(routeGroup, function(group, groupName) {
        ...
        _.each(routes, function(route) {
            ...
            switch (route.type) {
                case 'GET':
                    serv.get(url, attachData, route.func);
                    break;
                case 'POST':
                    serv.post(url, attachData, route.func);
                    break;
                case 'REST':
                    if (route.func.query) {
                        serv.get(url, attachData, route.func.query);
                    }
                    if (route.func.insert) {
                        serv.post(url, attachData, route.func.insert);
                    }
                    if (route.func.detail) {
                        serv.get(url+'/:id', attachData, route.func.detail);
                    }
                    if (route.func.patch) {
                        serv.post(url+'/:id', attachData, route.func.patch);
                        serv.patch(url+'/:id', attachData, route.func.patch);
                        serv.put(url+'/:id', attachData, route.func.patch);
                    }
                    if (route.func.del) {
                        serv.del(url+'/:id', attachData, route.func.del);
                    }
                default:
            }
        })
